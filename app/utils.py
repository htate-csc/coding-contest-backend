...
def send_email(
    *,
    ...
    if "@" not in email_to:
        logger.warning(f"Skipping email sending: '{email_to}' is not a valid email address (login_id mode). Subject: {subject}")
        return
...
def generate_new_account_email(
    ...
    project_name = settings.PROJECT_NAME
    subject = f"{project_name} - New account for user {login_id}"
    html_content = render_email_template(
