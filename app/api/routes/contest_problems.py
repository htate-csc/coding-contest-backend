...
from app.models import (
    ContestProblems,
    ContestProblemsCreate,
    ContestProblemsPublic,
    ContestProblemsUpdate,
    Message,
)
...
def create_contest_problems(
    *,
    ...
    ContestProblems_in: ContestProblemsCreate,
) -> Any:
    """
    ...
    db_contest_problems = ContestProblems.model_validate(ContestProblems_in)
...
def update_contest_problems(
    *,
    ...
    if not db_contest_problems:
        raise HTTPException(status_code=404, detail="ContestProblems not found")
    if not current_user.is_superuser:
        raise HTTPException(status_code=403, detail="Not enough permissions")
